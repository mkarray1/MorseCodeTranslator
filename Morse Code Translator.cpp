...
//utility functions
void inputs(char store[], int size);
...
void translateToMorse(char result[501][8]);			//translates
...
State cont()
{
	...
	char input[11];
	State ans;
	cout<<(stdout,"Are you sending or receiving? Enter something asides from \"sending\" and \"receiving\" to quit: ");
	...
	else if(!(strcmp("receiving",input)) || !(strcmp("receiving\n", input)))
	{
		ans = receiving;
	}
...
void validInput(char input[])
{
	...
	while(!valid)
	{
		...
		if(!valid || index>=500)
			cout<<"Please enter a valid input (Only 500 uppercase characters, numbers, and spaces allowed)"<<endl;
	}
}
bool startOutput()
{
...
void outputMorse(char result[501][8])
{
	fw.write("STARTING TO TRANSLATE MORSE TO ENGLISH", 9);	
	int index=0;
	codepair currCode;
...
void translateToMorse(char result[501][8])
{
